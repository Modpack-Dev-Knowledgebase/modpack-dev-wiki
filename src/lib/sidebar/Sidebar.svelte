...
<aside
    ...
    {#if windowInfo.isNavOpen}
      ...
      <div class="flex items-center mb-2 gap-1">
        <button
          class="{page == 'wiki'
            ? 'bg-stone-700'
            : 'bg-stone-800'} cursor-pointer hover:text-white hover:font-medium px-2 py-1 rounded-md flex items-center gap-1 focus-visible:outline-2 focus-visible:outline-mdw-teal"
          onclick={() => {
            page = "wiki";
  ...
  <div class="flex text-sm text-stone-600 p-3 items-center w-full">
    ...
    <button
      ...
      class="hidden sm:block rounded-lg cursor-pointer text-stone-200 text-lg hover:bg-stone-700 hover:text-white motion-safe:transition-all focus-visible:outline-2 focus-visible:outline-mdw-teal {windowInfo.isNavOpen
        ? 'rotate-0'
        : 'rotate-180'}"
